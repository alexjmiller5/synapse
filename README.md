...
- **Google Cloud Functions**:
  - **`ingestion-function`**: HTTP-triggered API endpoint for categorizing data and sending it to Notion.
  - **`reporting-function`**: Cron-triggered (Cloud Scheduler) for twice-daily email summaries.
...
```bash
uv run functions_framework --target=ingestion_function --debug
```

or

```bash
uv run functions_framework --target=reporting_function --debug
```
