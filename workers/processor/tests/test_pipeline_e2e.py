...
from main import run_pipeline, processor
from helpers import make_gemini_response, make_cloud_event, make_notion_page
...
class TestTaskPipeline:
    def test_simple_task(self, mock_gemini, mock_notion):
        _setup_classify_extract_mobile(mock_gemini, "tasks", {
            "Name": "Update hinge profile",
            "AI Title": "Update Hinge profile",
            "Tags": ["Chore"],
            "Due Date": "2026-03-29",
        })

        _run(_item("Update hinge profile"))
...
class TestBookmarkPipeline:
    def test_new_bookmark(self, mock_gemini, mock_notion):
        ...
        with patch("external_data.fetch_web_metadata", return_value="HTML Title: DevTool\nContent..."):
    ...
    def test_github_bookmark_auto_tagged(self, mock_gemini, mock_notion):
        ...
        with patch("external_data.fetch_web_metadata", return_value="HTML Title: Repo\nContent..."):
...
class TestPeoplePipeline:
    def test_new_person(self, mock_gemini, mock_notion):
        _setup_classify_extract(mock_gemini, "people", {
            "Name": "Rishi Patel",
            "Company": "TDP Senior Associate",
        })

        _run(_item("Rishi TDP senior associate"))
        mock_notion.pages.create.assert_called()
...
class TestPodcastPipeline:
    def test_spotify_podcast(self, mock_gemini, mock_notion):
        ...
        with patch("external_data.get_spotify_metadata", return_value="Show: My Show\nEp: Great Episode\nDesc: Good"):
...
class TestProcessorEntryPoint:
    def test_batch_processing(self, mock_gemini, mock_notion):
        """The processor() function should decode base64, parse, and run pipeline for each item."""
        # Mock parse_raw_input to return 2 items
        with patch("main.parse_raw_input") as mock_parse, \
             patch("main.hydrate_dynamic_options"), \
             patch("main.fetch_active_projects", return_value=(["Synapse"], {"Synapse": "id"})), \
             patch("main.fetch_inventory_map", return_value={}), \
             patch("main.fetch_trips_inventory", return_value=([], {})):
            ...
            event = make_cloud_event("Buy milk $ groceries @ Call John")
            processor(event)
