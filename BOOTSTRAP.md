...
The project's CI/CD pipeline (`terraform.yml`) is designed to authenticate using a GCP Service Account (`terraform-sa`) and store its state in a GCS bucket (`...-terraform-state`).
...
2. **Prepared Local Environment:**
    ...
    eval $(./scripts/setup-local-env.sh)
    ```
...
5. **Migrated State to GCS:**
    ...
    terraform init -migrate-state \
      -backend-config="bucket=$(yq e '.gcp_project_id' ../config.yml)-terraform-state" \
